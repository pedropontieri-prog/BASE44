...
export default function PsychologistProfile() {
  ...
  async function loadPsychologist() {
    ...
    try {
      ...
      console.error('Erro inesperado ao carregar psicólogo:', error);
      setP(null);
    } finally {
      setLoading(false);
    }
  ...
  const toggleFav = async () => {
    try {
      ...
      console.error('Erro inesperado ao alterar favorito:', error);
    }
  };
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="w-28 h-28 rounded-3xl overflow-hidden bg-muted shrink-0">
                  {photo ? (
                    ...
                    <div className="w-full h-full gradient-brand-soft flex items-center justify-center text-4xl font-heading font-bold text-primary">
                      {name.charAt(0)}
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1">
                  ...
                  {(p.crp_region || p.crp_number) && (
                    <p className="text-sm text-muted-foreground mt-1">
                      CRP {p.crp_region || ''}/{p.crp_number || ''}
                    </p>
                  )}
            ...
            <Section title="Modalidades de atendimento">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {modalities.includes('online') && (
                  <span
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-aqua/10 text-sm font-medium"
                    style={{ color: 'hsl(178 60% 35%)' }}
                  >
                    <Video size={16} />
                    Online
                  </span>
                ...
                {(modalities.includes('in_person') ||
                  ...
                  <span className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-soft text-primary text-sm font-medium">
                    <Building2 size={16} />
                    Presencial
                    {p.address ? ` · ${p.address}` : ''}
                  </span>
          ...
          <div className="lg:col-span-1">
            ...
            <div className="card-elevated p-6 sticky top-24">
              ...
              <div className="mt-5">
                ...
                <label className="text-xs font-medium text-muted-foreground">
                  Horários — {p.timezone || 'America/Sao_Paulo'}
                </label>
                ...
                <div className="grid grid-cols-3 gap-2 mt-1.5">
                  ...
                  {slots.map(s => (
                    <button
                      ...
                      className={`px-2 py-2.5 rounded-xl text-sm font-medium border transition-all ${
                        selectedSlot === s
                          ? 'gradient-brand text-white border-transparent shadow-soft'
                          : 'border-border text-foreground/70 hover:bg-muted'
                      }`}
              ...
              >
                {selectedSlot
                  ? `Confirmar ${days[selectedDay]} ${selectedSlot}`
              ...
              <button
                onClick={toggleFav}
                className={`w-full mt-3 py-3 rounded-full text-sm font-semibold border transition-all inline-flex items-center justify-center gap-2 ${
                  fav
                    ? 'text-rose-500 border-rose-200 bg-rose-50 dark:bg-rose-500/10'
                    : 'border-border text-foreground/70 hover:bg-muted'
                }`}
              >
                <Heart
                  size={16}
                  className={fav ? 'fill-rose-500' : ''}
                />
              ...
              <div className="mt-5 flex items-start gap-2 text-xs text-muted-foreground bg-muted/50 rounded-xl p-3">
                ...
                <span>
                  ...
                  Sessão de {p.session_duration || 50} min.
                </span>
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((t, i) => (
        <span
          key={i}
          className="text-xs px-2.5 py-1 rounded-full bg-violet-soft text-primary font-medium"
        >
          {t}
        </span>
      ))}
